...
(function () {
  ...
  const RULES = {
    ...
    // ---- lab-derived rules (from the bloods you entered) ----
    lab_anaemia: { avoid:[T.highInt], caution:[T.aerobic],
      note:"Your haemoglobin is low: less oxygen is reaching the muscle, so you will be breathless and tired at efforts you'd normally find easy — that's the anaemia, not lack of effort. Keep intensity light, judge by the talk test rather than pace, and get the cause treated; exercise capacity usually rebounds once it is." },
    lab_anaemia_severe: { avoid:[T.highInt, T.aerobic, T.impact], clearance:true,
      note:"Your haemoglobin is severely low. Get medical review BEFORE exercising — at this level exertion strains the heart, and treating the cause comes first. Gentle range of motion only until you're cleared." },
    lab_bleeding: { avoid:[T.impact], caution:[T.balance, T.highInt],
      note:"Your clotting results (INR and/or platelets) mean you bruise and bleed more easily and a fall matters more than usual. Impact and contact work is removed, balance work should be done near support, and any significant knock — especially to the head — needs same-day medical review." },
    lab_electrolyte: { avoid:[T.highInt], caution:[T.aerobic],
      note:"Your potassium is outside the normal range, which affects heart rhythm — and exercise shifts potassium further. Keep effort light until it's corrected, and report palpitations, dizziness or unusual weakness promptly." },
    lab_ckd: { avoid:[], caution:[T.highInt],
      note:"Your kidney filtration (eGFR) is reduced. Exercise is genuinely good for kidney disease, but build gradually, keep well hydrated, and be cautious with NSAIDs and with exercising in the heat — both add kidney strain." },
