...
4. **Updates:** there is **no Watchtower**. `:latest` does not move by itself.
   Pull and recreate the container (Portainer **Re-pull image and redeploy**, or
   `docker pull` + recreate). Pass `XAI_API_KEY` so Jeffery can use Grok via the
   same-origin `/xai/` proxy — the key is never in the PWA JavaScript.
